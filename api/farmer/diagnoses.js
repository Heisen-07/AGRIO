/**
 * GET  /api/farmer/diagnoses — get authenticated farmer's leaf diagnosis history
 * POST /api/farmer/diagnoses — save leaf diagnosis and advisory for authenticated farmer
 */
import { connectDB } from '../lib/db.js';
import { DiagnosisRecord } from '../lib/models.js';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST') {
      const { farmId = 'demo_farm', data, thumbnail, timestamp } = req.body || {};
      if (!data) {
    ...
    return res.status(500).json({ error: 'Failed to process diagnosis record.' });
  }
}
